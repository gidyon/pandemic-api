...
// RegisterQuestionnaireAPI registers http router for the suspect API
func RegisterQuestionnaireAPI(router *httprouter.Router, sqlDB *gorm.DB) {
	// Validation
	var err error
	switch {
	case sqlDB == nil:
	...
	// Auto migration
	err = questionnaireCase.sqlDB.AutoMigrate(&Questionnaire{}).Error
	handleError(err)

	router.GET("/rest/v1/cases/questionnaire/:caseId", questionnaireCase.GetQuestionnaire)
	router.GET("/rest/v1/cases/questionnaire", questionnaireCase.ListQuestionnaires)
	router.POST("/rest/v1/cases/questionnaire", questionnaireCase.AddQuestionnaire)
	router.PATCH("/rest/v1/cases/questionnaire/:caseId/attend", questionnaireCase.MarkCaseAttended)
}

// Questionnaire represent a COVID19 questionnaire case
...
func (*Questionnaire) BeforeCreate(scope *gorm.Scope) error {
...
func (suspect *Questionnaire) validate() error {
	var err error
	switch {
...
func (sap *questionnaireCase) AddQuestionnaire(w http.ResponseWriter, r *http.Request, _ httprouter.Params) {
	// Marshaling
...
func (sap *questionnaireCase) ListQuestionnaires(w http.ResponseWriter, r *http.Request, _ httprouter.Params) {
	query := r.URL.Query()
	// Get filters
	...
	// Execute query
	suspects := make([]*Questionnaire, 0, ps)
...
func (sap *questionnaireCase) GetQuestionnaire(w http.ResponseWriter, r *http.Request, p httprouter.Params) {
	// Get suspect id
	caseID := p.ByName("caseId")
	if strings.TrimSpace(caseID) == "" {
		http.Error(w, "missing case id", http.StatusBadRequest)
